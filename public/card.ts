const render = async () => {
    ...
    if (parsed.facebook) content += `<p><a href="${parsed.facebook}">Facebook</a></p>`
    if (parsed.twitter) content += `<p><a href="${parsed.twitter}">Twitter</a></p>`
    if (parsed.linkedin) content += `<p><a href="${parsed.linkedin}">LinkedIn</a></p>`
    if (parsed.github) content += `<p><a href="${parsed.github}">GitHub</a></p>`
    if (parsed.tumblr) content += `<p><a href="${parsed.tumblr}">Tumblr</a></p>`
    if (parsed.portfolio) content += `<p><a href="${parsed.portfolio}">Portfolio</a></p>`
